...
//  SC Watch Watch App
//
//  Created by Ryan Forsyth on 2023-08-20.
//
...
extension Image {
    ...
    var averageColor: UIColor {
        let uiImage = getUIImage()
        return uiImage?.findAverageColor(algorithm: .simple) ?? .clear
    }
    
    @MainActor
    func getUIImage(newSize: CGSize = CGSize(width: 100, height: 100)) -> UIImage? {
        let image = resizable()
            .scaledToFill()
